...
CREATE TABLE IF NOT EXISTS utente (
    ...
    utente_id VARCHAR(100) NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS professore (
    nome VARCHAR(100) NOT NULL,
    professore_id VARCHAR(100) NOT NULL PRIMARY KEY,
    immagineprofilo VARCHAR(255) NULL
) ENGINE=InnoDB;

-- Corsi
CREATE TABLE IF NOT EXISTS corso (
    corso_id VARCHAR(100) NOT NULL PRIMARY KEY, 
    nome VARCHAR(100) NOT NULL
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS insegnamento (
    professore_id VARCHAR(100) NOT NULL,
...
CREATE TABLE IF NOT EXISTS appello (
  ...
  CONSTRAINT fk_app_prof FOREIGN KEY (professore_id)
    REFERENCES professore(professore_id)
    ON DELETE CASCADE ON UPDATE CASCADE,

  CONSTRAINT fk_app_corso FOREIGN KEY (corso_id)
    REFERENCES corso(corso_id)
    ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE IF NOT EXISTS recensione (
    ...
    PRIMARY KEY (utente_id, professore_id, corso_id, anno_accademico, data_appello),
    ...
    CONSTRAINT fk_rec_app FOREIGN KEY (professore_id, corso_id, anno_accademico, data_appello)
    REFERENCES appello(professore_id, corso_id, anno_accademico, data_appello)
        ON DELETE CASCADE ON UPDATE CASCADE
